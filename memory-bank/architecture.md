...
- 操作按钮区域现在包含“新对局”按钮。
- 新对局按钮使用 `data-action="new-game"` 作为事件绑定标识。
- 新对局按钮不依赖当前对局状态禁用，允许在对局进行中或结束后重开。
- 摸牌、胡牌、吃、碰、杠按钮继续沿用各自现有启用状态；新对局按钮不改变规则判断。

设计约束：

- `render.ts` 只负责输出按钮 DOM，不直接调用 `createNewGameState()`。
- 新对局重置逻辑由 `src/ui/events.ts` 处理，避免渲染层修改游戏状态。
...
- 在既有事件委托中处理 `data-action="new-game"` 的按钮点击。
- 点击新对局后调用 `createNewGameState()`，生成全新的对局状态。
- 将事件闭包中的 `currentState` 替换为新对局状态。
- 调用 `renderApp(container, currentState)` 完整重绘页面。

设计约束：

- 新对局不手动清空手牌、牌河、副露、结束结果、结算记录或点数，而是完全复用 `createNewGameState()` 的初始化结果。
- 当前基础单局版本中，新对局会将玩家与电脑点数重置为 100000。
- 新对局不会保留上一局记录，也不会累计点数。
...
- `createNewGameState()` 是 UI 重开对局的唯一状态入口。
- 新状态会重新生成牌池、洗牌、发牌，并初始化玩家、电脑、牌山、当前行动方、最后弃牌、结束结果、结算记录、海底和河底标记。
- UI 不应自行拼装新对局状态，避免遗漏字段或形成重复初始化逻辑。
...
- 对局结束后点击新对局可以重新开始，并清除结束提示与点数变化面板。
...
第 27 步当前只补充测试覆盖，原则上不改变模块职责。

已涉及的测试文件职责补充：

- `src/game/tile.test.ts`：保护基础牌 ID、排序、同牌判断和未知 ID 错误边界。
- `src/game/deck.test.ts`：保护完整牌池组成、洗牌确定性、洗牌不修改原数组，以及洗牌后实体牌身份守恒。
- `src/game/player.test.ts`：保护玩家状态实例隔离，尤其是 `hand`、`discardPile`、`melds` 和 `points` 互不共享。
- `src/game/game-state.test.ts`：保护新对局初始化字段，包括 `scoreSettlement: null` 和 48 张实体牌守恒。
- `src/game/draw.test.ts`：保护摸牌、海底、空牌山流局，以及已结束状态不再摸牌。
- `src/game/discard.test.ts`：保护合法打牌、非法打牌、最后弃牌记录、河底标记，以及已结束状态不再打牌。
- `src/game/turn.test.ts`：保护合法弃牌后的回合切换，以及非法弃牌不会触发电脑荣和或副露响应。
- `src/game/ai.test.ts`：保护电脑确定性弃牌策略，包括孤立牌优先、保留对子/刻子和同牌副本选择顺序。
- `src/game/rules.test.ts`：保护标准形、七对子、非法牌、牌数边界，以及 `isBasicWinningHand()` 的统一入口行为。
- `src/game/ron.test.ts`：保护有役判断、无役不可胡、荣和入口、电脑自动荣和、河底和副露役牌。
- `src/game/tsumo.test.ts`：保护自摸入口、电脑自动自摸、海底摸月、自摸结算和结束状态保护。
- `src/game/meld.test.ts`：保护吃、碰、明杠、暗杠候选和声明入口，以及响应窗口关闭等非法路径。
- `src/game/scoring.test.ts`：保护固定分值结算、流局零分差、二人零和，以及无结束结果时不生成结算。

### 当前发现的架构疑点

- `src/game/ron.ts` 的 `evaluateWinningHand()` 当前对七对子只返回 `seven-pairs`，即使七对子全部由 2-8 索子组成，也没有同时返回 `tanyao`。
- 按日麻规则，七对子与断幺九可以复合；因此第 27 步后续应核对 `hasTanyao()` 或七对子役种组合逻辑。
- 该疑点属于核心规则实现问题，不应仅通过降低测试期望来掩盖。

### 后续衔接边界

- 第 27 步尚未完成；后续应先修复或明确七对子与断幺九复合役行为，再由用户运行全量测试和构建确认。
- 后续仍应保持测试覆盖在测试文件中，规则修复集中在 `src/game/ron.ts` 或必要的规则辅助函数中，不应写入 UI 层。
- 测试断言应使用当前 Vitest/Chai 已支持的 API；当前项目中集合大小断言应使用 `expect(set.size).toBe(n)`，不要使用未配置类型支持的 `toHaveSize()`。
